...
class StackSet {
  ...
  withForceMode(force) {
    if (force)
      this.forceMode = true;
    this.logger.info(`Force mode : ${this.forceMode}`);
    return this;
  }
  ...
  withFailureToleranceCount(failureToleranceCount) {
    ...
    this.logger.info(`Failure Tolerance Count : ${this.failureToleranceCount}`);
    return this;
  }

  withMaxConcurrencyCount(maxConcurrentCount) {
    if (maxConcurrentCount)
      this.maxConcurrentCount = maxConcurrentCount;
    this.logger.info(`Max Concurrent count : ${this.maxConcurrentCount}`);
    return this;
  }

  async createOrUpdate(filePath) {
    this.body = await this.readFile(filePath);
    // TODO : add mode with some specific accounts only
    // TODO : add mode of update without body (no change but update it)
    //        for this mode, TemplateBody must not be replaced by 'UsePreviousTemplate: true'
    const params = {
    ...
    if (stackSet) {
      ...
      if (this.forceMode || !this.isEqual(stackSet)) {
  ...
  async updateStackInstances(accounts) {
    ...
    if (newAccountsToManage.length > 0) {
      ...
      // TODO : find the currently in error stack instances
